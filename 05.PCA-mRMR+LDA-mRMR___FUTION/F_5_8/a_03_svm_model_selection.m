...
uniqueClass = unique(CV_trainClass_60_F_5_8);
...
for cVal = cMinValue : 1 : cMaxValue
   ...
   for sigmaVal = sigmaMinValue : 1 : sigmaMaxValue
      ...
      for i=1:size(uniqueClass,1)
          groups = ismember(CV_trainClass_60_F_5_8, uniqueClass(i));
          warning('off', 'stats:obsolete:ReplaceThisWith');
          opts=svmsmoset('MaxIter',500000,'KernelCacheLimit',30000);
          svmStruct(i) = svmtrain(CV_trainData_60_F_5_8,groups,'Method','SMO','SMO_Opts',opts,'Kernel_Function','rbf','BoxConstraint', actualC,'RBF_Sigma',actualSigma);
      end
      
      prediction = predictTestClass(svmStruct, CV_testData_40_F_5_8, uniqueClass);
    
      [overallAccuracy, accuracyPerClass] = performanceMeasure(prediction, CV_testClass_40_F_5_8);
      
      toc
      sprintf('C = %f, Sigma = %f, Ove_Acc = %f', cVal, sigmaVal, overallAccuracy)
      accuracyPerClass
      tmp = [cVal sigmaVal overallAccuracy];
      F_5_8_Result = [F_5_8_Result; tmp];
   end
end

endtime = toc(starttime)
save F_5_8_Result.mat F_5_8_Result
...
load F_5_8_Result
[~,idx] = sort(F_5_8_Result(:,3),'descend');
sorted_F_5_8_Result = F_5_8_Result(idx,:);
save sorted_F_5_8_Result.mat sorted_F_5_8_Result
